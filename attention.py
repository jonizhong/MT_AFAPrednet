...
class Attention(nn.Module):
    ...
    def forward(self, state, action):
        ...
        input_size = action.size()[1] # motor action values
        num_classes = len(state) # possible predictions
        ...
        yarray = torch.stack(state)
        
        
        yarray = yarray.permute(1,2,3,4,0)
         
        
        #print torch.mm(yarray, out.permute(1,0)).size()
        
        result = torch.mm(yarray.view(-1,num_classes),out.permute(1,0))
        
         
        
        result = result.view(yarray.size()[0:4])
